...
5. Nouveautés de cette version : même page de localisation de la version iOS, champ obligatoire par langue — voir la section dédiée plus bas dans ce fichier (à adapter à chaque sortie ; version actuelle du dépôt : 1.2.1+19).
...
*Sur la page de la version iOS (même localisation que Nom / Description) : champ « Nouveautés de cette version » ; en interface anglaise : What’s New in This Version. À remplir pour chaque langue de la version. Adapte le texte ci‑dessous à chaque release (réf. dépôt : 1.2.1+19).*

• Effets sonores en partie plus fiables dès le début d’une run (notamment sur Android).
• Stabilité au démarrage et lors de la sauvegarde en ligne ; correctifs et optimisations.
...
• Sons de jeu plus fiables (Android) ; stabilité sync ; correctifs (v. 1.2.1).
